...
def build_parser() -> argparse.ArgumentParser:
    parser = argparse.ArgumentParser(description="Build a refined within-community-board matched NYC comparison from the enriched panel.")
...
def write_plot(year_summary: pd.DataFrame, output_path: Path) -> None:
    ...
    plt.plot(year_summary["inspection_year"], year_summary["mean_gap"], linewidth=2.5, label="Refined matched treated-control gap")
    plt.axhline(0, color="black", linewidth=1, alpha=0.5)
    plt.title("NYC HPD Violations: Refined Within-Community-Board Matched Gap")
...
def main() -> None:
    ...
    matches = match_treated_to_controls(
        features,
        exact_match_cols=("borough", "communityboard", "yearbuilt_bin", "units_bin", "pre_nonzero_years", "pre_mean_bin"),
    ...
    matches_path = processed_dir / f"nyc_within_cb_refined_matches_since_{args.since_year}.csv"
    matched_panel_path = processed_dir / f"nyc_within_cb_refined_matched_panel_since_{args.since_year}.csv"
    balance_path = results_dir / f"nyc_within_cb_refined_match_balance_since_{args.since_year}.csv"
    year_gap_path = results_dir / f"nyc_within_cb_refined_year_gap_summary_since_{args.since_year}.csv"
    coef_path = results_dir / f"nyc_within_cb_refined_gap_model_coefficients_since_{args.since_year}.csv"
    model_path = results_dir / f"nyc_within_cb_refined_gap_model_summary_since_{args.since_year}.txt"
    figure_path = figures_dir / f"nyc_within_cb_refined_gap_since_{args.since_year}.png"
